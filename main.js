...
const jellyLight = new THREE.PointLight(0x168cff, 18, 24, 1.3);
...
jelly.scale.setScalar(1.25);
jelly.position.set(-0.8, -0.1, -2.6);
scene.add(jelly);
...
for (let i = 0; i < 170; i++) {
  ...
  const speck = new THREE.Mesh(
    new THREE.SphereGeometry(0.008 + Math.random() * 0.014, 8, 6),
    new THREE.MeshBasicMaterial({
...
function addTubeObject(kind, index, angle, radius, len, thickness, color, opacity) {
  ...
  mesh.userData = { kind, index, angle, radius, len, thickness, phase: Math.random() * 10, baseOpacity: opacity };
...
for (let i = 0; i < 28; i++) {
  ...
  jelly.add(new THREE.Mesh(geo, mat));
}
...
function updateTubeMesh(mesh, time, breath) {
  ...
  const pts = [];
  const count = d.kind === 'oral' ? 14 : 15;

  for (let j = 0; j < count; j++) {
    const t = j / (count - 1);
    const lag = t * (d.kind === 'oral' ? 2.8 : 3.25);
    const slow = d.kind === 'hair' ? 0.68 : 1.0;

    const sCurve = Math.sin(time * 0.34 + d.phase - lag + d.index * 0.071) * (0.04 + t * 0.42) * slow;
    const counter = Math.sin(time * 0.16 + d.phase * 0.7 + t * 6.2) * (0.02 + t * 0.16) * slow;
    const depthWave = Math.cos(time * 0.30 + d.phase * 1.2 - lag * 0.7) * (0.03 + t * 0.22) * slow;

    const angleDrift = d.angle + Math.sin(time * 0.08 + d.phase) * 0.08;
    const baseRadius = d.kind === 'oral' ? d.radius + Math.sin(t * 3.4 + time * 0.12) * 0.04 : d.radius * (1 - t * 0.12);

    const sideX = Math.cos(angleDrift + Math.PI / 2);
    const sideZ = Math.sin(angleDrift + Math.PI / 2);

    const x = Math.cos(angleDrift) * baseRadius + sideX * (sCurve + counter);
    const y = (d.kind === 'oral' ? -0.18 : -0.04) - t * d.len + Math.sin(time * 0.20 + d.phase - lag) * 0.07 * t - breath * 0.03;
    const z = Math.sin(angleDrift) * baseRadius + sideZ * depthWave + Math.sin(time * 0.11 + t * 5.0 + d.phase) * 0.055 * t;
  ...
  mesh.geometry = new THREE.TubeGeometry(curve, d.kind === 'oral' ? 56 : 64, d.thickness, d.kind === 'oral' ? 7 : (d.kind === 'hair' ? 3 : 4), false);
}
...
renderer.domElement.addEventListener('pointermove', (e) => {
  ...
  if (activePointers.size >= 2) {
    ...
    if (pinchStartDistance > 0 && currentDistance > 0) {
      ...
      controls.targetDistance = THREE.MathUtils.clamp(pinchStartZoom * scale, 3.6, 8.0);
    }
    return;
  }
...
renderer.domElement.addEventListener('wheel', (e) => {
  e.preventDefault();
  controls.targetDistance = THREE.MathUtils.clamp(controls.targetDistance + e.deltaY * 0.0024, 3.6, 8.0);
}, { passive: false });
...
function animate() {
  ...
  jellyTarget.set(
    Math.sin(swimTime * 0.105) * 0.72 + Math.sin(swimTime * 0.031) * 0.28,
    -0.12 + Math.sin(time * 0.145) * 0.20 + Math.sin(time * 0.049) * 0.08 + breath * 0.06 - recoil * 0.02,
    -2.35 + Math.cos(swimTime * 0.079) * 0.26
  );
  jelly.position.lerp(jellyTarget, 0.018);

  jelly.rotation.y += (Math.sin(time * 0.085) * 0.30 - jelly.rotation.y) * 0.018;
